...
## Step 11: Secrets scanning — the gap, the tool, the blind spots, the fix

### The problem: Git never forgets

A **secret** = any string that grants access (API keys, DB passwords,
cloud keys, tokens). Committing one is uniquely bad:

1. **History is forever.** Deleting it in the next commit doesn't remove
   it — every clone carries every version of every file. The only real
   fix is **rotation** (invalidate + reissue).
2. **Bots hunt public repos.** Scanners scrape fresh GitHub commits for
   key patterns; real cloud keys get abused within minutes (crypto-mining
   fleets, five-figure bills).
3. **Private repos don't save you** — anyone/anything with access reads
   it (see axios remediation: rotate everything in .env and CI/CD).

Two defense lines: **pre-commit hook** (best: secret never enters
history) + **pipeline scan** (safety net). We built the pipeline line
first.

### Proving the gap (learn-by-breaking)

Planted a fake config in `labs/app-1/src/config.ts`:
...
(Keys are AWS's official documentation examples — safe for a public repo,
but shaped like real keys. The "TODO: move later" comment is a museum
piece found next to leaked keys in countless real breaches.)

Pushed with an innocent message ("Add app config") → **pipeline fully
green.** Why every stage missed it:

- npm audit checks dependency VERSIONS against CVEs — our code isn't a
  dependency
- tsc checks TYPES — a hardcoded string is valid TypeScript
- nothing read code CONTENT for secret patterns

**Green pipeline ≠ secure code. The pipeline only catches what it's told
to look for.**

### Adding gitleaks (the current standard OSS secrets scanner)

Step added to ci.yml right after checkout (fail fast — no reason to
install Node before leak-checking):
...
- `${{ ... }}` = Actions expression syntax, filled at runtime
- `secrets.GITHUB_TOKEN` = temporary per-run token auto-generated by
  GitHub (lets the action attach findings to the run; expires with the
  job). Irony: a properly managed secret detecting improperly managed
  ones.

### Blind spot #1: diff-based scanning

First gitleaks run: GREEN with the AWS key already in history. Log showed
`git log -p -1` → it scanned only THE COMMITS IN THIS PUSH. Our key
entered two pushes earlier → outside the window.

**Diff scanning protects the future, not the past.** Efficient for CI,
but pre-existing leaks stay invisible unless you sweep history.

Proof detection works for new leaks: added a fake GitHub PAT
(`ghp_` + 36 chars) to config.ts, pushed → RED. Findings table showed
rule IDs (github-pat + generic-api-key — one string can match multiple
rules), commit, line, author, email, date = leak forensics for free.

Bonus finding: gitleaks-action@v2 itself triggers the Node 20 deprecation
warning — security tooling is ALSO a dependency that ages. No newer major
yet; noted as watch-and-wait. (Future pattern: run the gitleaks binary
directly instead of the wrapper action.)

### Blind spot #2: allowlists

Full-history sweep locally:

```bash
brew install gitleaks
gitleaks git .        # scan entire git history
gitleaks git . -v     # verbose: full finding details
```

Result: 19 commits scanned, but ONLY the ghp_ token found (twice — two
rules). The buried AWS key: absent. Reason: gitleaks' default config
ALLOWLISTS known dummy values, and AKIAIOSFODNN7EXAMPLE is THE canonical
AWS docs key. Scanners exclude these to avoid false-positive noise from
docs/tests/tutorials.

Lessons:

1. Know your scanner's allowlist — deliberate blind spots exist.
   **A green scan means "nothing matched the rules," never "no secrets
   exist."**
2. False-positive management is a design feature of every security tool
   (we'll meet it again in SAST tuning).

Proof realistic keys ARE caught (no git, no commit — `gitleaks dir`
scans plain files):

```bash
echo "aws_key = '<FAKE_AWS_KEY_AKIA_FORMAT>'" > /tmp/leak-test.txt
gitleaks dir /tmp/leak-test.txt -v
# → leaks found: 1 (RuleID: generic-api-key, entropy 4.22)
```

Detail: it matched generic-api-key (entropy + "aws_key =" context), not
the dedicated AWS rule — detection is layered and rule-dependent; verify,
don't assume.

### The right way: environment variables + .env pattern

Three pieces:

**1. `labs/app-1/.env`** — real values, LOCAL ONLY (already in .gitignore
since day one — that decision paid off today):
...
**2. `labs/app-1/.env.example`** — COMMITTED template, documents which
variables exist, contains NO values. New teammate: copy to .env, fill in
own values. The shape is shared; the secrets never are.
...
**3. `src/config.ts`** — references without containing:
...
`process.env` = Node's window into the process environment, injected
from outside (shell / .env loader in dev, the platform in production).
`??` = fallback if unset (fine for non-secrets like region).

Verified before committing: `git status -uall` showed config.ts modified,
.env.example untracked-visible, **.env completely absent** — the
invisible line IS the control working.

Pushed → gitleaks green (diff only removes secret-shapes now).

Not done yet (deliberate): nothing loads .env into process.env yet
(modern Node: `--env-file` flag — coming when the app actually needs the
values).

### Doctrine: rotation vs history rewrite

With REAL leaked keys, two moves in strict priority order:

1. **ROTATE — always, immediately, non-negotiable.** Treat the key as
   stolen the moment it's pushed. Invalidate at the provider, reissue,
   deliver via .env/secret manager. Post-rotation, the string in history
   is a dead key.
2. **History rewrite — optional, situational, painful.** git filter-repo
   can erase strings from all commits, BUT: all later commit hashes
   change, clones break, force-push required, platform caches may retain
   old objects. Worth it for compliance/embarrassment/unrotatable keys.
   Never a substitute for rotation.

**"Rotation kills the secret; rewriting only hides the corpse."**

Our repo: keys are fake + documented teaching artifacts → history stays.
